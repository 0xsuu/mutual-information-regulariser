...
def kl_info_bottleneck_reg_mws_id_exp(mu_id, log_var_id, mu_exp, log_var_exp,
                                      dataset_size, beta=50., neutral_mask=None):
    """
    ...
    p_id_dist = torch.distributions.Normal(torch.zeros_like(mu_id), torch.ones_like(sigma_id))  # Prior distribution.
    p_exp_dist = torch.distributions.Normal(torch.zeros_like(mu_exp), torch.ones_like(sigma_exp))  # Prior distribution.

    z_id = q_z_id_x_dist.rsample()
    z_exp = q_z_exp_x_dist.rsample()

    kl_loss = torch.distributions.kl_divergence(q_z_id_x_dist, p_id_dist).sum(1) + \
        torch.distributions.kl_divergence(q_z_exp_x_dist, p_exp_dist).sum(1)

    if beta == 0.:
        return z_id, z_exp, kl_loss
    ...
    log_q_z_id = (log_sum_exp(_log_q_z_id.sum(2), dim=1, keepdim=False) - math.log(batch_size * dataset_size))

    kl_info_bottleneck_reg = kl_loss
    if neutral_mask is not None:
        exp_mask = torch.logical_not(neutral_mask)
        kl_info_bottleneck_reg[exp_mask] += beta * (log_q_z_id_x - log_q_z_id)[exp_mask]
    else:
        kl_info_bottleneck_reg += beta * (log_q_z_id_x - log_q_z_id)

    return z_id, z_exp, kl_info_bottleneck_reg
